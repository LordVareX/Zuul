...
/**
 * <p>A single player attribute value to be used when making a
 * StartMatchBackfill request.  This could be a string, number, string list, or
 * string/number map. For more information, see the <a
...
public:
   ...
   */
  explicit AttributeValue(double n)
      : m_N(n), m_S(""), m_attrType(AttrType::DOUBLE) {}

  /**
   * <p>Construct an attribute value of type string.</p>
   */
  AttributeValue(std::string s)
      : m_N(0), m_S(s), m_attrType(AttrType::STRING) {}
  ...
  AttributeValue(const AttributeValue &other)
      : m_S(other.m_S), m_N(other.m_N), m_SL(other.m_SL), m_SDM(other.m_SDM),
  ...
  inline void AddString(std::string value) {
    if (m_attrType == AttrType::STRING_LIST &&
        m_SL.size() < MAX_STRING_LIST_LENGTH) {
  ...
  inline void AddStringAndDouble(std::string key, double value) {
    if (m_attrType == AttrType::STRING_DOUBLE_MAP &&
        m_SDM.size() < MAX_STRING_MAP_SIZE) {
...
private:
  explicit AttributeValue(AttrType attrType)
      : m_N(0), m_S(""), m_attrType(attrType) {}
#else
public:
  explicit AttributeValue(AttrType attrType)
      : m_N(0), m_SLCount(0), m_SDMCount(0), m_attrType(attrType) {
    m_S[0] = '\0';
  }

  AttributeValue()
      : m_N(0), m_SLCount(0), m_SDMCount(0), m_attrType(AttrType::NONE) {
    m_S[0] = '\0';
  }
   ...
   */
  explicit AttributeValue(double n)
      : m_N(n), m_SLCount(0), m_SDMCount(0), m_attrType(AttrType::DOUBLE) {
    m_S[0] = '\0';
  }
  ...
  inline void AddString(const char *value) {
    if (m_attrType == AttrType::STRING_LIST &&
        m_SLCount < MAX_STRING_LIST_LENGTH) {
  ...
  inline void AddStringAndDouble(const char *key, double value) {
    if (m_attrType == AttrType::STRING_DOUBLE_MAP &&
        m_SDMCount < MAX_STRING_MAP_SIZE) {
