...
/**
 * Class modeling universal resource identifier. Member variables for this class
 * can only be set via Builder.
 */
class Uri final {
public:
  class UriBuilder {
  public:
    ...
    /**
     * Add the base URL. This includes any part of the URI except the query
     * string.
     */
   ...
   */
  inline const std::map<std::string, std::string> &GetQueryMap() const {
    return m_queryMap;
  }

  inline bool operator==(const Uri &other) const {
    return other.GetUriString() == GetUriString();
  }

  friend std::ostream &operator<<(std::ostream &os, const Uri &uri);

private:
  Uri(const std::string &baseUriString,
