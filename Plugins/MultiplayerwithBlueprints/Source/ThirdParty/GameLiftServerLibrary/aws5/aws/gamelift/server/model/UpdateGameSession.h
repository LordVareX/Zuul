...
public:
  UpdateGameSession(const GameSession &gameSession, UpdateReason updateReason,
                    std::string backfillTicketId)
      : m_backfillTicketId(backfillTicketId), m_gameSession(gameSession),
  ...
  /**
   * <p>The ticketId used for the submitted match backfill request that this
   * update is in response to.  Empty if this update was not in response to a
   * backfill.</p>
   */
...
public:
  UpdateGameSession(const GameSession &gameSession, UpdateReason updateReason,
                    const char *backfillTicketId)
      : m_gameSession(gameSession), m_updateReason(updateReason) {
    strncpy(m_backfillTicketId, backfillTicketId,
            MAX_BACKFILL_TICKET_ID_LENGTH - 1);
  ...
  UpdateGameSession(const UpdateGameSession &other)
      : m_gameSession(other.m_gameSession),
        m_updateReason(other.m_updateReason) {
    strncpy(m_backfillTicketId, other.m_backfillTicketId,
            sizeof(m_backfillTicketId));
  }
  ...
  UpdateGameSession &operator=(const UpdateGameSession &other) {
    ...
    m_updateReason = other.m_updateReason;
    strncpy(m_backfillTicketId, other.m_backfillTicketId,
            sizeof(other.m_backfillTicketId));
