...
public:
  WebSocketPlayerSession()
      : m_creationTime(-1), m_terminationTime(-1), m_status(), m_port(-1) {}

  ~WebSocketPlayerSession() = default;
  WebSocketPlayerSession(const WebSocketPlayerSession &other) = default;
  WebSocketPlayerSession(WebSocketPlayerSession &&other) {
    *this = std::move(other);
  }

  WebSocketPlayerSession &
  operator=(const WebSocketPlayerSession &other) = default;
  ...
  inline void SetCreationTime(const long creationTime) {
  ...
  inline void SetTerminationTime(const long terminationTime) {
    m_terminationTime = terminationTime;
  }

  inline WebSocketPlayerSession &
  WithTerminationTime(const long terminationTime) {
  ...
  inline void SetStatus(const WebSocketPlayerSessionStatus status) {
    m_status = status;
  }

  inline WebSocketPlayerSession &
  WithStatus(const WebSocketPlayerSessionStatus status) {
    SetStatus(status);
    return *this;
  }
  ...
  inline void SetPlayerData(const std::string &playerData) {
