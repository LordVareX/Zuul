...
public:
  ...
  inline const WebSocketAttrType &GetAttributeType() const {
    return m_attrType;
  }

  inline void SetAttributeType(const WebSocketAttrType &attrType) {
    m_attrType = attrType;
  }

  inline WebSocketAttributeValue &
  WithAttributeType(const WebSocketAttrType &attrType) {
  ...
  inline WebSocketAttributeValue &
  WithSDM(const std::map<std::string, double> &sdm) {
    SetSDM(sdm);
    return *this;
  }
