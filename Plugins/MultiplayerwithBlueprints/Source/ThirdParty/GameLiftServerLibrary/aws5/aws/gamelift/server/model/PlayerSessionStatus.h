...
inline AWS_GAMELIFT_API PlayerSessionStatus
GetPlayerSessionStatusForName(const std::string &name) {
  if (name == "RESERVED") {
...
GetNameForPlayerSessionStatus(PlayerSessionStatus value) {
  switch (value) {
...
#else
inline AWS_GAMELIFT_API PlayerSessionStatus
GetPlayerSessionStatusForName(const char *name) {
