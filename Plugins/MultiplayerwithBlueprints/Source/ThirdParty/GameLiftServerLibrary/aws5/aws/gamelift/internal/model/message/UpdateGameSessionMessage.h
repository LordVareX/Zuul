...
public:
  ...
  operator=(const UpdateGameSessionMessage &) = default;
  UpdateGameSessionMessage &operator=(UpdateGameSessionMessage &&) = default;
  ~UpdateGameSessionMessage() = default;

  inline const WebSocketGameSession &GetGameSession() const {
    return m_gameSession;
  }

  inline const std::string &GetUpdateReason() const { return m_updateReason; }

  inline const std::string &GetBackfillTicketId() const {
    return m_backfillTicketId;
  }

  inline void SetGameSession(WebSocketGameSession &gameSession) {
    m_gameSession = gameSession;
  }

  inline void SetUpdateReason(const std::string &updateReason) {
    m_updateReason = updateReason;
  }

  inline void SetBackfillTicketId(const std::string &backfillTicketId) {
    m_backfillTicketId = backfillTicketId;
  }

  inline UpdateGameSessionMessage &
  WithGameSession(WebSocketGameSession &gameSession) {
    SetGameSession(gameSession);
    return *this;
  }

  inline UpdateGameSessionMessage &
  WithUpdateReason(const std::string &updateReason) {
    SetUpdateReason(updateReason);
    return *this;
  }

  inline UpdateGameSessionMessage &
  WithBackfillTicketId(const std::string &backfillTicketId) {
  ...
  operator<<(std::ostream &os,
             const UpdateGameSessionMessage &createGameSessionMessage);
