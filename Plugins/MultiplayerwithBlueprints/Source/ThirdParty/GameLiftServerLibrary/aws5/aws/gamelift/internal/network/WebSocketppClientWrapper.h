...
public:
  WebSocketppClientWrapper(
      std::shared_ptr<WebSocketppClientType> webSocketClient);

  Aws::GameLift::GenericOutcome Connect(const Uri &uri) override;
  Aws::GameLift::GenericOutcome
  SendSocketMessage(const std::string &requestId,
                    const std::string &message) override;
  void Disconnect() override;
  void RegisterGameLiftCallback(
      const std::string &gameLiftEvent,
...
private:
  ...
  std::map<std::string, std::function<GenericOutcome(std::string)>>
      m_eventHandlers;
  ...
  Aws::GameLift::GenericOutcome
  SendSocketMessageAsync(const std::string &message);
  ...
  websocketpp::lib::shared_ptr<asio::ssl::context>
