...
public:
  ...
  AWS_GAMELIFT_API LogParameters(const std::vector<std::string> &logPaths)
      : m_logPaths(logPaths) {}
  ...
  // Move Constructor
  AWS_GAMELIFT_API LogParameters(LogParameters &&o)
  ...
  AWS_GAMELIFT_API std::vector<std::string> getLogPaths() const {
    return m_logPaths;
  }

  AWS_GAMELIFT_API int getLogPathCount() const { return m_logPaths.size(); }

  AWS_GAMELIFT_API const char *getLogPath(int index) const {
    return index < m_logPaths.size() ? m_logPaths[index].c_str() : nullptr;
  }

  AWS_GAMELIFT_API bool operator==(const LogParameters &other) const {
    return getLogPaths() == other.getLogPaths();
  }
...
public:
  ...
  AWS_GAMELIFT_API const char *getLogPath(int index) const {
    return index < m_count ? m_logPaths[index] : nullptr;
  }
