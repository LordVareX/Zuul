...
public:
  ...
  inline const std::map<std::string, WebSocketAttributeValue> &
  GetPlayerAttributes() const {
    return m_playerAttributes;
  }

  inline void SetPlayerAttributes(
      const std::map<std::string, WebSocketAttributeValue> &playerAttributes) {
    m_playerAttributes = playerAttributes;
  }

  inline WebSocketPlayer &WithPlayerAttributes(
      const std::map<std::string, WebSocketAttributeValue> &playerAttributes) {
  ...
  inline void SetLatencyInMs(const std::map<std::string, int> &latencyInMs) {
    m_latencyInMs = latencyInMs;
  }

  inline WebSocketPlayer &
  WithLatencyInMs(const std::map<std::string, int> &latencyInMs) {
