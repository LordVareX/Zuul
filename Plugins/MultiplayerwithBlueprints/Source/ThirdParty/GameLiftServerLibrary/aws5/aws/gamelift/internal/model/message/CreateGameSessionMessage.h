...
public:
  CreateGameSessionMessage() : m_maximumPlayerSessionCount(-1), m_port(-1) {
    SetAction(CREATE_GAME_SESSION);
  };
  ...
  operator=(const CreateGameSessionMessage &) = default;
  ...
  inline const std::string &GetGameSessionName() const {
    return m_gameSessionName;
  }
  ...
  inline const std::map<std::string, std::string> &GetGameProperties() const {
    return m_gameProperties;
  }

  inline void SetGameSessionId(const std::string &gameSessionId) {
    m_gameSessionId = gameSessionId;
  }

  inline void SetGameSessionName(const std::string &gameSessionName) {
    m_gameSessionName = gameSessionName;
  }

  inline void SetGameSessionData(const std::string &gameSessionData) {
    m_gameSessionData = gameSessionData;
  }

  inline void SetMatchmakerData(const std::string &matchmakerData) {
    m_matchmakerData = matchmakerData;
  }
  ...
  inline void
  SetMaximumPlayerSessionCount(const int maximumPlayerSessionCount) {
    m_maximumPlayerSessionCount = maximumPlayerSessionCount;
  }

  inline void SetPort(const int port) { m_port = port; }

  inline void
  SetGameProperties(const std::map<std::string, std::string> &gameProperties) {
    m_gameProperties = gameProperties;
  }

  inline CreateGameSessionMessage &
  WithGameSessionId(const std::string &gameSessionId) {
    SetGameSessionId(gameSessionId);
    return *this;
  }

  inline CreateGameSessionMessage &
  WithGameSessionName(const std::string &gameSessionName) {
    SetGameSessionName(gameSessionName);
    return *this;
  }

  inline CreateGameSessionMessage &
  WithGameSessionData(const std::string &gameSessionData) {
    SetGameSessionData(gameSessionData);
    return *this;
  }

  inline CreateGameSessionMessage &
  WithMatchmakerData(const std::string &matchmakerData) {
  ...
  inline CreateGameSessionMessage &
  WithMaximumPlayerSessionCount(const int maximumPlayerSessionCount) {
  ...
  inline CreateGameSessionMessage &
  WithGameProperties(const std::map<std::string, std::string> &gameProperties) {
  ...
  operator<<(std::ostream &os,
             const CreateGameSessionMessage &createGameSessionMessage);

protected:
  virtual bool
  Serialize(rapidjson::Writer<rapidjson::StringBuffer> *writer) const;
...
private:
  ...
  static constexpr const char *MAXIMUM_PLAYER_SESSION_COUNT =
