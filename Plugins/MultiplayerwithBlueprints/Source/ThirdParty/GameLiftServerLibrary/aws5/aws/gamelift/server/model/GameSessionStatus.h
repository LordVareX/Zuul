...
inline AWS_GAMELIFT_API GameSessionStatus
GetGameSessionStatusForName(const std::string &s_name) {
  const char *name = s_name.c_str();
#else
inline AWS_GAMELIFT_API GameSessionStatus
GetGameSessionStatusForName(const char *name) {
#endif
...
GetNameForGameSessionStatus(GameSessionStatus value)
#else
