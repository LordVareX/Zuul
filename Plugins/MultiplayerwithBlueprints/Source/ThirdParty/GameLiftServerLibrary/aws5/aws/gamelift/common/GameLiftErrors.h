...
enum class AWS_GAMELIFT_API GAMELIFT_ERROR_TYPE {
  ALREADY_INITIALIZED, // The GameLift Server or Client has already been
                       // initialized with Initialize().
  FLEET_MISMATCH, // The target fleet does not match the fleet of a gameSession
                  // or playerSession.
  GAMELIFT_CLIENT_NOT_INITIALIZED, // The GameLift client has not been
                                   // initialized.
  GAMELIFT_SERVER_NOT_INITIALIZED, // The GameLift server has not been
                                   // initialized.
  GAME_SESSION_ENDED_FAILED, // The GameLift Server SDK could not contact the
                             // service to report the game session ended.
  GAME_SESSION_NOT_READY, // The GameLift Server Game Session was not activated.
  GAME_SESSION_READY_FAILED, // The GameLift Server SDK could not contact the
                             // service to report the game session is ready.
  ...
  INITIALIZATION_MISMATCH,   // A client method was called after
                             // Server::Initialize(), or vice versa.
  NOT_INITIALIZED, // The GameLift Server or Client has not been initialized
                   // with Initialize().
  NO_TARGET_ALIASID_SET, // A target aliasId has not been set.
  NO_TARGET_FLEET_SET,   // A target fleet has not been set.
  PROCESS_ENDING_FAILED, // The GameLift Server SDK could not contact the
                         // service to report the process is ending.
  PROCESS_NOT_ACTIVE,    // The server process is not yet active, not bound to a
                         // GameSession, and cannot accept or process
                         // PlayerSessions.
  PROCESS_NOT_READY,     // The server process is not yet ready to be activated.
  PROCESS_READY_FAILED, // The GameLift Server SDK could not contact the service
                        // to report the process is ready.
  ...
  STX_INITIALIZATION_FAILED, // The XStx server backend component has failed to
                             // initialize.
  UNEXPECTED_PLAYER_SESSION, // An unregistered player session was encountered
                             // by the server.
  BAD_REQUEST_EXCEPTION,
  INTERNAL_SERVICE_EXCEPTION,
  WEBSOCKET_CONNECT_FAILURE, // Failure to connect to the GameLift Service
                             // WebSocket
  WEBSOCKET_RETRIABLE_SEND_MESSAGE_FAILURE, // Retriable failure to send message
                                            // to the GameLift Service WebSocket
  WEBSOCKET_SEND_MESSAGE_FAILURE // Failure to send message to the GameLift
                                 // Service WebSocket
};
...
public:
  ...
  static std::string
  GetDefaultMessageForErrorType(GAMELIFT_ERROR_TYPE errorType) {
    switch (errorType) {
    case GAMELIFT_ERROR_TYPE::ALREADY_INITIALIZED:
      return "GameLift has already been initialized. You must call Destroy() "
             "before "
             "reinitializing the client or server.";
    case GAMELIFT_ERROR_TYPE::FLEET_MISMATCH:
      return "The Target fleet does not match the request fleet. Make sure "
             "GameSessions and "
             "PlayerSessions belong to your target fleet.";
    case GAMELIFT_ERROR_TYPE::GAMELIFT_CLIENT_NOT_INITIALIZED:
      return "The GameLift client has not been initialized. Please call "
             "SetTargetFleet or "
             "SetTArgetAliasId.";
    case GAMELIFT_ERROR_TYPE::GAMELIFT_SERVER_NOT_INITIALIZED:
      return "The GameLift server has not been initialized. Please call "
             "InitSDK.";
    case GAMELIFT_ERROR_TYPE::GAME_SESSION_ENDED_FAILED:
    ...
    case GAMELIFT_ERROR_TYPE::INITIALIZATION_MISMATCH:
      return "The current call does not match the initialization state. Client "
             "calls require "
             "a call to Client::Initialize(), and Server calls require "
             "Server::Initialize(). "
             "Only one may be active at a time.";
    case GAMELIFT_ERROR_TYPE::NOT_INITIALIZED:
      return "GameLift has not been initialized! You must call "
             "Client::Initialize() or "
             "Server::InitSDK() before making GameLift calls.";
    case GAMELIFT_ERROR_TYPE::NO_TARGET_ALIASID_SET:
      return "The aliasId has not been set. Clients should call "
             "SetTargetAliasId() before "
             "making calls that require an alias.";
    case GAMELIFT_ERROR_TYPE::NO_TARGET_FLEET_SET:
      return "The target fleet has not been set. Clients should call "
             "SetTargetFleet() before "
    ...
    case GAMELIFT_ERROR_TYPE::PROCESS_NOT_READY:
      return "The process has not yet been activated by calling "
             "ProcessReady(). Processes in "
    ...
    case GAMELIFT_ERROR_TYPE::SDK_VERSION_DETECTION_FAILED:
      return "Could not detect SDK version.";
    case GAMELIFT_ERROR_TYPE::SERVICE_CALL_FAILED:
      return "An AWS service call has failed. See the root cause error for "
             "more information.";
    case GAMELIFT_ERROR_TYPE::STX_CALL_FAILED:
    ...
    case GAMELIFT_ERROR_TYPE::UNEXPECTED_PLAYER_SESSION:
      return "The player session was not expected by the server. Clients "
             "wishing to connect "
             "to a server must obtain a PlayerSessionID from GameLift by "
             "creating a player "
  ...
  GameLiftError(const int statusCode, const std::string &message)
      : GameLiftError(GetErrorTypeForStatusCode(statusCode), message){};

  GameLiftError(GAMELIFT_ERROR_TYPE errorType)
      : m_errorType(errorType),
        m_errorName(Aws::GameLift::GameLiftError::GetDefaultNameForErrorType(
            errorType)),
        m_errorMessage(
            Aws::GameLift::GameLiftError::GetDefaultMessageForErrorType(
                errorType)){};

  GameLiftError(GAMELIFT_ERROR_TYPE errorType, const std::string &errorName,
                const std::string &message)
      : m_errorType(errorType), m_errorName(errorName),
  ...
  GameLiftError(const GameLiftError &rhs)
      : m_errorType(rhs.GetErrorType()), m_errorName(rhs.GetErrorName()),
        m_errorMessage(rhs.GetErrorMessage()){};

  bool operator==(const GameLiftError other) const {
    return other.GetErrorType() == GetErrorType() &&
           other.GetErrorName() == GetErrorName() &&
...
public:
  ...
  GameLiftError(const int statusCode, const char *message)
      : GameLiftError(GetErrorTypeForStatusCode(statusCode), message){};

  void Init(GAMELIFT_ERROR_TYPE errorType, const char *errorName,
            const char *message) {
    m_errorType = errorType;
  ...
  GameLiftError(GAMELIFT_ERROR_TYPE errorType, const char *errorName,
                const char *message)
      : m_errorType(errorType) {
  ...
  bool operator==(const GameLiftError other) const {
    return other.GetErrorType() == GetErrorType() &&
           strcmp(other.GetErrorName(), GetErrorName()) == 0 &&
...
private:
  ...
  static GAMELIFT_ERROR_TYPE GetErrorTypeForStatusCode(const int statusCode) {
    if (statusCode >= 400 && statusCode < 500) {
      // Map all 4xx requests to bad request exception. We don't have an error
      // type for all 100 client errors
      return GAMELIFT_ERROR_TYPE::BAD_REQUEST_EXCEPTION;
    } else {
      // The websocket can return other error types, in this case classify it as
