...
namespace Internal {
/**
 * Message from GameLift indicating the SDK should refresh its websocket
 * connection.
 */
...
public:
  ...
  operator=(const RefreshConnectionMessage &) = default;
  RefreshConnectionMessage &operator=(RefreshConnectionMessage &&) = default;
  ~RefreshConnectionMessage() = default;

  inline const std::string &GetRefreshConnectionEndpoint() const {
    return m_refreshConnectionEndpoint;
  }
  inline const std::string &GetAuthToken() const { return m_authToken; }

  inline void
  SetRefreshConnectionEndpoint(const std::string &refreshConnectionEndpoint) {
    m_refreshConnectionEndpoint = refreshConnectionEndpoint;
  }
  inline void SetAuthToken(const std::string &authToken) {
    m_authToken = authToken;
  }

  inline RefreshConnectionMessage &
  WithRefreshConnectionEndpoint(const std::string &refreshConnectionEndpoint) {
  ...
  operator<<(std::ostream &os,
             const RefreshConnectionMessage &refreshConnectionMessage);
...
private:
  ...
  static constexpr const char *REFRESH_CONNECTION_ENDPOINT =
