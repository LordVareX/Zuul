...
class GameLiftServerState : public GameLiftCommonState,
                            public IGameLiftMessageHandler {
public:
  static constexpr const char *LANGUAGE = "Cpp";

private:
  static constexpr const char *ENV_VAR_WEBSOCKET_URL =
  ...
  static constexpr const int HEALTHCHECK_TIMEOUT_MILLIS =
      HEALTHCHECK_INTERVAL_MILLIS - HEALTHCHECK_MAX_JITTER_MILLIS;

  void GetOverrideParams(char **webSocketUrl, char **authToken,
                         char **processId, char **hostId, char **fleetId);
  void ReportHealth();
  void HealthCheck();
...
public:
  static Server::InitSDKOutcome CreateInstance(
      std::shared_ptr<IWebSocketClientWrapper> webSocketClientWrapper);

  virtual GAMELIFT_INTERNAL_STATE_TYPE GetStateType() override {
    return GAMELIFT_INTERNAL_STATE_TYPE::SERVER;
  };

  // Singleton constructors should be private, but we are using a custom
  // allocator that needs to be able to see them. Don't use these.
  GameLiftServerState();
  ...
  GenericOutcome InitializeNetworking(
      const Aws::GameLift::Server::Model::ServerParameters &serverParameters);

  GenericOutcome ActivateGameSession();

  GenericOutcome UpdatePlayerSessionCreationPolicy(
      PlayerSessionCreationPolicy newPlayerSessionPolicy);
  ...
  GenericOutcome
  StopMatchBackfill(const Aws::GameLift::Server::Model::StopMatchBackfillRequest
  ...
  void OnRefreshConnection(const std::string &refreshConnectionEndpoint,
...
#else
public:
  static Internal::InitSDKOutcome CreateInstance(
      std::shared_ptr<IWebSocketClientWrapper> webSocketClientWrapper);
...
private:
  std::function<void(Aws::GameLift::Server::Model::GameSession, void *)>
      m_onStartGameSession;
  std::function<void(Aws::GameLift::Server::Model::UpdateGameSession, void *)>
      m_onUpdateGameSession;
...
#endif
public:
  GetFleetRoleCredentialsOutcome GetFleetRoleCredentials(
...
private:
  ...
  std::unique_ptr<DescribePlayerSessionsCallback>
      m_describePlayerSessionsCallback;
  std::unique_ptr<GetComputeCertificateCallback>
      m_getComputeCertificateCallback;
  std::unique_ptr<GetFleetRoleCredentialsCallback>
      m_getFleetRoleCredentialsCallback;
  ...
  // Assume we're on managed EC2, if GetFleetRoleCredentials fails we know to
  // set this to false
  bool m_onManagedEC2 = true;
  std::map<std::string, GetFleetRoleCredentialsResult>
      m_instanceRoleResultCache;
