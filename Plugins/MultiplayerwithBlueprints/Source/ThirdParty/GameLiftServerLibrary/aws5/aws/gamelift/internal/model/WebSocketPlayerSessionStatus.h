...
enum class WebSocketPlayerSessionStatus {
  NOT_SET,
  RESERVED,
  ACTIVE,
  COMPLETED,
...
public:
  inline static WebSocketPlayerSessionStatus
  GetStatusForName(const std::string &name) {
    if (name == RESERVED) {
  ...
  GetNameForStatus(WebSocketPlayerSessionStatus value) {
    switch (value) {
